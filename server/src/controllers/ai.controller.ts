...
async function fetchPortfolioPayload(userId: string) {
  const portfolio = await prisma.portfolio.findFirst({
    where: { userId, familyMemberId: null },
    include: {
      holdings: {
        where: { isActive: true },
        select: { id: true, assetClass: true, symbol: true, name: true, currentValue: true, totalInvested: true, pnlPercent: true }
      }
    }
  });
  
  if (!portfolio) {
    return { holdings: [], currentValue: 0 };
  }
  
  return {
    holdings: (portfolio.holdings as any[]).map(h => ({
      id: h.id, assetClass: h.assetClass, symbol: h.symbol, name: h.name,
      currentValue: Number(h.currentValue ?? 0),
      totalInvested: Number(h.totalInvested ?? 0),
      pnlPercent: Number(h.pnlPercent ?? 0)
    })),
    currentValue: Number(portfolio.currentValue ?? 0)
  };
}

export const optimizePortfolio = wrap(async (req, res) => {
  const payload = await fetchPortfolioPayload(req.user!.id);
  try {
    const resp = await axios.post(`${AI_SERVICE_URL}/ai/optimize-portfolio`, payload);
    ok(res, resp.data);
  } catch (err) {
...
export const riskMetrics = wrap(async (req, res) => {
  const payload = await fetchPortfolioPayload(req.user!.id);
  try {
    const resp = await axios.post(`${AI_SERVICE_URL}/ai/risk-metrics`, payload);
    ok(res, resp.data);
  } catch (err) {
    res.status(502).json({ success: false, message: 'AI service error' });
  }
...
export const customRiskScore = wrap(async (req, res) => {
  const payload = await fetchPortfolioPayload(req.user!.id);
  try {
    const resp = await axios.post(`${AI_SERVICE_URL}/ai/custom-risk-score`, payload);
    ok(res, resp.data);
  } catch (err) {
    res.status(502).json({ success: false, message: 'AI service error' });
  }
...
export const correlation = wrap(async (req, res) => {
  const payload = await fetchPortfolioPayload(req.user!.id);
  try {
    const resp = await axios.post(`${AI_SERVICE_URL}/ai/correlation`, payload);
    ok(res, resp.data);
  } catch (err) {
    res.status(502).json({ success: false, message: 'AI service error' });
  }
...
export const monteCarlo = wrap(async (req, res) => {
  const payload = await fetchPortfolioPayload(req.user!.id);
  ...
  try {
    const resp = await axios.post(`${AI_SERVICE_URL}/ai/monte-carlo`, {
      portfolio: payload,
      goal: {
        id: goal.id,
