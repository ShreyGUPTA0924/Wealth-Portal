...
import {
  ...
  AlertTriangle, BriefcaseBusiness,
} from 'lucide-react';
...
function SymbolCombobox({ assetType, onSelect, error }: SymbolComboboxProps) {
  ...
  const doSearch = useCallback(async (q: string) => {
    ...
    setLoading(true);
    try {
      // Server returns: { success: true, data: { results: SearchResult[] } }
  ...
  const handleBlur = async () => {
    // Close dropdown after short delay (allow click to register)
    setTimeout(() => setOpen(false), 200);

    // If user typed something but didn't select from dropdown, accept as free text
    const trimmed = query.trim();
    if (trimmed && !selected) {
      const rawSymbol = trimmed.split('—')[0]!.trim().toUpperCase();
      onSelect(rawSymbol, rawSymbol);

      // Verify symbol by fetching price
      try {
  ...
  return (
    <Field label="Symbol / Scheme" error={error}>
