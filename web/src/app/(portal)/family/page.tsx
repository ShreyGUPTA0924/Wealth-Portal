      ...
      <div className="flex items-center gap-4 flex-wrap">
        {allocation.map((a, i) => (
          <div key={a.memberId} className="flex items-center gap-1.5">
            ...
            <span className="text-xs text-gray-600">{a.fullName}</span>
            <span className="text-xs font-medium text-gray-900">{a.percent.toFixed(1)}%</span>
          </div>
        ))}
      </div>
    </div>
        ...
        <div className="space-y-4">
          <div>
            ...
            <input
              ...
              className="mt-1 w-full px-3 py-2.5 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-[#3C3489] transition-colors"
          ...
          <div>
            ...
            <div className="relative mt-1">
              <select
                ...
                className="w-full appearance-none px-3 py-2.5 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-[#3C3489] transition-colors bg-white"
              >
          ...
          {isChild && (
            <div>
              <label className="text-xs font-medium text-gray-500 uppercase tracking-wide">Monthly Allowance (₹)</label>
              <input
                type="number"
    ...
    <div className="bg-white rounded-2xl border border-gray-100 p-5 hover:shadow-sm transition-shadow">
      ...
      {portfolio ? (
        <div className="space-y-2 mb-4">
          <div className="flex justify-between items-center">
            ...
            <span className="text-sm font-bold text-gray-900">{formatInr(portfolio.currentValue, true)}</span>
          </div>
          <div className="flex justify-between items-center">
          ...
          {member.monthlyAllowance && (
            <div className="flex justify-between items-center">
              ...
              <span className="text-sm font-medium text-gray-700">{formatInr(member.monthlyAllowance)}</span>
            </div>
          )}
          <div className="flex justify-between items-center">
            <span className="text-xs text-gray-400">Holdings</span>
            <span className="text-xs text-gray-600">{portfolio.holdingsCount} assets</span>
          </div>
        </div>
      ) : (
        <div className="py-3 text-center text-xs text-gray-400 mb-3">No portfolio data yet</div>
      )}
      ...
      <div className="flex gap-2">
        ...
        {member.isMinor ? (
          <button
            disabled
            className="px-3 py-2 rounded-xl border border-gray-200 text-gray-400 text-xs flex items-center gap-1 cursor-not-allowed"
          ...
          <Link
            href={`/family/${member.id}`}
            className="px-3 py-2 rounded-xl border border-gray-200 text-gray-600 text-xs hover:bg-gray-50 transition-colors"
          >
            + Holding
          </Link>
        )}
      ...
      <div className="space-y-5">
        ...
        {members.length === 0 ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 text-center">
            ...
            <h2 className="text-lg font-semibold text-gray-900 mb-2">Your family vault is empty</h2>
            <p className="text-sm text-gray-500 mb-1">Add family members to track everyone's wealth together</p>
            <p className="text-sm text-gray-400 mb-6">Manage portfolios for spouse, children, and parents</p>
            <button
              onClick={() => setShowAddModal(true)}
          ...
          <>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-6">
              <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-5">
                <div>
                  <p className="text-xs font-medium text-gray-400 uppercase tracking-wider mb-1">Family Net Worth</p>
                  <p className="text-4xl font-bold text-gray-900">{formatInr(overview.familyNetWorth)}</p>
                ...
                <div className="text-right">
                  ...
                  <p className="text-lg font-semibold text-gray-700">{formatInr(overview.totalInvested, true)}</p>
                </div>
              </div>

              <div>
                <p className="text-xs text-gray-400 mb-2">Contribution by member</p>
